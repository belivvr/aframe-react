...
function isTablet(mockUserAgent?: string): boolean {
...
function isFirefoxReality(): boolean {
  return /(Mobile VR)/i.test(window.navigator.userAgent);
}

function isMobileVR(): boolean {
...
function isMobile(): boolean {
  const device = window.navigator.userAgent || window.navigator.vendor;
...
export interface CursorProps {
  /**
   * Array of additional events on the entity to listen to for triggering mousedown
   * (e.g., `triggerdown` for vive-controls).
   */
  downEvents?: string[];
  ...
  /**
   * Array of additional events on the entity to _listen_ to for triggering mouseup
   * (e.g., `trackpadup` for daydream-controls).
   */
  upEvents?: string[];
